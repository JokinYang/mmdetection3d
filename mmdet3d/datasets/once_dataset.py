...
class OnceDataset(Det3DDataset):
    r"""ONCE Dataset.
    ...
    def __init__(self,
        ...
        self.is_label_rotatez90cw = True
        # Check PointsRotateZ90CW is in the pipeline.
        assert not any(
            map(lambda p: True if isinstance(p, PointsRotateZ90CW) or str(p) == 'PointsRotateZ90CW' else False,
                pipeline)), "PointsRotateZ90CW should not be in the pipeline and behind LoadPointsFromFile."
    ...
    def parse_ann_info(self, info: dict) -> dict:
        ...
        if self.is_label_rotatez90cw:
            m = np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]])
            gt_bboxes_3d.rotate(m.T)
