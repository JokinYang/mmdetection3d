...
def update_once_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['sample_idx'] = ori_info_dict['frame_id']

        temp_data_info['lidar_points']['num_pts_feats'] = ori_info_dict[
            'point_cloud']['num_features']
        temp_data_info['lidar_points']['lidar_path'] = str(Path(ori_info_dict['point_cloud']['velodyne_path']))

        anns = ori_info_dict.get('annos', None)
        ignore_class_name = set()
        if anns is not None:
            # (quat_x, quat_y, quat_z, quat_w, trans_x, trans_y, trans_z).
            pose = ori_info_dict['pose']
            trans_xyz = np.array(pose[-3:])
            num_instances = len(anns['name'])
            instance_list = []
            for instance_id in range(num_instances):
                empty_instance = get_empty_instance()
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'once':
        update_once_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
