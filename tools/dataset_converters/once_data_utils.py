...
def get_once_ann_info(seq_id, prefix='data'):
    prefix = Path(prefix)
    anno_file_path = prefix / seq_id / '{}.json'.format(seq_id)
    anno_dict = json.loads(anno_file_path.read_bytes())
    return anno_dict


def get_once_image_info(path,
                        image_ids=(),
                        extend_matrix=True,
                        num_worker=8,
                        relative_path=True,
    ...
    def map_func(seq_id):
        if isinstance(seq_id, int):
            seq_id = "{:06d}".format(seq_id)

        ann_info = get_once_ann_info(seq_id, str(root_path / 'data'))
        data_list = []
        for i in range(len(ann_info['frames'])):
            frame = ann_info['frames'][i]
            velodyne_path = osp.join('data', seq_id, 'lidar_roof', frame['frame_id'] + '.bin')
            if not relative_path:
                velodyne_path = (root_path / velodyne_path).absolute()

            pc_info = {'num_features': 4, 'velodyne_path': velodyne_path}
            pose = frame['pose']
            frame_id = frame['frame_id']

            annos = None
            # 有些frame没有标注。跳过没有标注的frame
            if frame.get('annos', None) is None:
                continue
                # pass
            else:
                annos = {}
                ...
                annos['dimensions'] = np.array([box[3:6] for box in boxes_3d]).reshape(-1, 3)
                annos['location'] = np.array([box[:3] for box in boxes_3d]).reshape(-1, 3)
                # trans center_z to bottom_z
                annos['location'][:, 2] = annos['location'][:, 2] - annos['dimensions'][:, 2] / 2

                annos['rotation_y'] = np.array([box[6] for box in boxes_3d]).reshape(-1)
                annos['name'] = np.array(names)
                annos['index'] = np.array(list(range(num_gt)), dtype=np.int32)
                annos['group_ids'] = np.arange(num_gt, dtype=np.int32)

            info = {
                'sequence_id': frame['sequence_id'],
                # 'prev_id': -1 if i > 0 else frames[i - 1]['frame_id'],
                # 'next_id': -1 if i < len(frames) - 1 else frames[i + 1]['frame_id'],
                'frame_id': frame_id,
                'pose': pose,
                'point_cloud': pc_info,
            }
    ...
    return list(itertools.chain(*image_infos))
...
if __name__ == '__main__':
    ...
    dataset = ONCE('/data/once')
    pc = dataset.load_point_cloud('000076', '1616343527700').reshape(-1, 4)[:, :3]

    visualizer = Det3DLocalVisualizer()

    visualizer.set_points(pc)
    visualizer.show()
